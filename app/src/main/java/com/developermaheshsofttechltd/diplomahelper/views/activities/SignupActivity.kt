...
//        googleSignInAccount = GoogleSignIn.getLastSignedInAccount(this) ?: return

        ivEtConfirmPassword.setOnClickListener {
            togglePasswordVisibility(etConfirmPassword, ivEtConfirmPassword)
        }

        ivEtPassword.setOnClickListener {
            togglePasswordVisibility(etPassword, ivEtPassword)
        }
...
//            }
    ...
    private fun togglePasswordVisibility(editText: EditText, imageView: ImageView) {
        if (editText.transformationMethod == HideReturnsTransformationMethod.getInstance()) {
            ...
            imageView.setImageResource(R.drawable.icon_eye_enable)
        } else {
            editText.transformationMethod = HideReturnsTransformationMethod.getInstance()
            imageView.setImageResource(R.drawable.icon_eye_disable)
        }
    }
    ...
    private fun validateAndSignup() {
        when {
            ...
            !etName.text.toString()
                .matches("[a-zA-Z ]+".toRegex()) -> showError("Please Enter Alphabets Only", etName)

            etName.text.length < 8 -> showError("Name should be of greater than 8 char..", etName)
